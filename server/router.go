...
func initRouter(dep Dependency)(router *mux.Router){
	...
	router.HandleFunc("/users/{id}",user.Authorize(user.FindById(dep.UserService),1)).Methods(http.MethodGet)
	...
	router.HandleFunc("/customer/{id}",user.Authorize(customer.DeleteCustomer(dep.CustomerService),1)).Methods(http.MethodDelete)
	...
	router.HandleFunc("/amountWithdraw",user.Authorize(transaction.AmountWithdraw(dep.transactionService),2)).Methods(http.MethodPost)
	router.HandleFunc("/amountDeposit",user.Authorize(transaction.AmmountDeposit(dep.transactionService),2)).Methods(http.MethodPost)
	return
}
